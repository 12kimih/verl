...
def make_map_fn(split):
    def process_fn(example, idx):
        ...
        docs = "\n".join(
            [
                f"Document [{doc['idx']}] (Title: {doc['title']}) {doc['paragraph_text']}"
                for doc in example["paragraphs"]
            ]
        )
        prompt_format = """
...
""".strip()
        ...
        messages = [
            {
                "role": "user",
                "content": prompt_format.format(question=question, context=docs),
            }
        ]
...
class MuSiQueDataset(Dataset):
    ...
    def _download(self, use_origin_parquet=False):
        ...
        for i, parquet_file in enumerate(data_files):
            ...
            self.data_files[i] = parquet_file
    ...
    def _read_files_and_tokenize(self):
        ...
        for parquet_file in self.data_files:
            ...
            if self.is_train:
                ...
                dataframe = datasets.load_dataset(parquet_file)["validation"]
                dataframe = dataframe.map(
                    function=make_map_fn("validation"), with_indices=True
                )

            # # for debugging
            # if self.is_train:
            #     dataframe = datasets.load_dataset(parquet_file)["train"]
            #     dataframe = dataframe.select(range(2000))
            #     dataframe = dataframe.map(function=make_map_fn('train'), with_indices=True)
            # elif not self.is_train:
            #     dataframe = datasets.load_dataset(parquet_file)["validation"]
            #     dataframe = dataframe.select(range(2000))
            #     dataframe = dataframe.map(function=make_map_fn('validation'), with_indices=True)
